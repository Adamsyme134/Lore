...
export function LinkWidget({ config }: LinkWidgetProps) {
  ...
  if (isInline) {
    return (
      ...
      >
        ...
        <AppText className="text-tertiary text-[10px] ml-1">↗</AppText>
      </Pressable>
    );
  }
  ...
  return (
    <Pressable 
      ...
      style={{ backgroundColor: cfg.bgImage ? '#000' : colors.surface, width: '100%' }} 
    >
      {!!cfg.bgImage && (
        <Image
      ...
      <View className="flex-1 ml-2 z-10">
        <AppText className="font-sansSemi text-base" style={{ color: cfg.textColor || (cfg.bgImage ? 'white' : colors.text) }}>
          {cfg.title || 'External Link'}
        </AppText>
        
        {!!cfg.desc && (
           <AppText className="text-xs mt-1" style={{ color: cfg.textColor ? `${cfg.textColor}CC` : (cfg.bgImage ? 'rgba(255,255,255,0.8)' : colors.textSecondary) }}>
            {cfg.desc}
          </AppText>
        )}
        ...
        {isAffiliate && (
          <AppText 
            ...
            style={{ color: cfg.textColor ? `${cfg.textColor}80` : (cfg.bgImage ? 'rgba(255,255,255,0.5)' : colors.textTertiary) }}
          >
            (i) we may earn a commission on payments made using this link
          </AppText>
        )}
      </View>
      <AppText className="z-10" style={{ color: cfg.textColor || (cfg.bgImage ? 'white' : colors.textTertiary) }}>↗</AppText>
    </Pressable>
  );
}
